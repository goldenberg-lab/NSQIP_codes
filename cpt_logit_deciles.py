...
for ii, vv in enumerate(cn_Y):
    ...
    # get deciles
    cpt_groups['decile'] = pd.qcut(cpt_groups['outcome_mean'], 5, labels=False)
...
res_y_all = pd.concat(holder_y_all).reset_index(drop=True)
res_y_all.to_csv(os.path.join(dir_output, 'logit_auc_cpt_within_quintiles.csv'), index=False)
