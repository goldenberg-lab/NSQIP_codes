    ...
    for yy in tmp_train_years:
        ...
        for cc in top_cpts:
            ...
            if all(sub_tmp_holder.y_values.values==0):
                ...
                within_holder.append(pd.DataFrame({'auc':metrics.roc_auc_score(list(sub_tmp_holder.y_values.values), list(sub_tmp_holder.y_preds.values)), 'cpt': cc}, index=[0]))
...
###############################################
# ---- STEP 3: LEAVE-ONE-YEAR - ALL VARIABLES (RISK SCORE INSTEAD OF CPT CODE) ---- #
#NEEDS TO BE FIXED
...
#for ii, vv in enumerate(cn_Y):
    ...
    #for yy in tmp_train_years:
        ...
        # predict
        #tmp_holder = pd.DataFrame({'y_preds': list(logit_preds), 'y_values':list(ytest.values),'cpt':list(tmp_cpt.cpt)})
        #within_holder = []
        #for cc in top_cpts:
           # print('cpt %s' % (cc))
            #sub_tmp_holder = tmp_holder[tmp_holder['cpt']==cc].reset_index(drop=True)
...
####################################################
# ---- STEP 4: LEAVE-ONE-YEAR - ALL VARIABLES, FOR EACH CPT CODE, SUB MODELS---- #
    ...
    for yy in tmp_train_years:
        ...
        for cc in top_cpts:
            ...
            if all(np.unique(sub_ytrain.values)==0) or all(np.unique(sub_ytest.values)==0):
                ...
                within_holder.append(pd.DataFrame({'auc':metrics.roc_auc_score(sub_ytest.values,logit_preds), 'cpt': cc}, index=[0]))
